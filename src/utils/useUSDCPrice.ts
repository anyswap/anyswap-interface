...
export default function useUSDCPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      ...
      [wrapped?.equals(BASEUSD) ? undefined : wrapped, chainId === ChainId.MAINNET ? BASEUSD : undefined],
      [chainId ? WETH[chainId] : undefined, chainId === ChainId.MAINNET ? BASEUSD : undefined]
    ],
  ...
  return useMemo(() => {
    ...
    // handle usdc
    if (wrapped.equals(BASEUSD)) {
      return new Price(BASEUSD, BASEUSD, '1', '1')
    }
    ...
    if (ethPairState === PairState.EXISTS && ethPair && usdcEthPairState === PairState.EXISTS && usdcEthPair) {
      if (usdcEthPair.reserveOf(BASEUSD).greaterThan('0') && ethPair.reserveOf(WETH[chainId]).greaterThan('0')) {
        const ethUsdcPrice = usdcEthPair.priceOf(BASEUSD)
        const currencyEthPrice = ethPair.priceOf(WETH[chainId])
        const usdcPrice = ethUsdcPrice.multiply(currencyEthPrice).invert()
        return new Price(currency, BASEUSD, usdcPrice.denominator, usdcPrice.numerator)
      }
    }
    return undefined
